...
cleanup() {
    tmux kill-session -t e2e5_test || true
...
set -g @tmst-custom-colorscheme-path "$PLUGIN_DIR/themes/catppuccin-mocha.conf"
source -F "$PLUGIN_DIR/tmux-style.conf"
EOF

# Start a new tmux session and load the plugin with test configurations
tmux -f /dev/null new-session -d -s e2e5_test

# Reload tmux environment
tmux source -F "$TMUX_CONF"

cat << EOF > "$EXPECTED_OUTPUT"
$PLUGIN_DIR/themes/catppuccin-mocha.conf
EOF
